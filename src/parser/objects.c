...
static bool	create_sphere(int id, t_scene **scene, char **arr, t_gc_object **gc)
{
	...
	if (!new_obj)
		return (print_error("Memory allocation failed for sphere", *gc),
			false);
	new_obj->type = SPHERE;
	new_obj->id = id;
...
static bool	create_plane(int id, t_scene **scene, char **arr, t_gc_object **gc)
{
	...
	new_obj = gc_alloc(sizeof(t_object), gc);
	if (!new_obj)
		return (print_error("Memory allocation failed for plane", *gc),
			false);
	new_obj->id = id;
	new_obj->type = PLANE;
...
static bool	create_cylinder(int id, t_scene **scn, char **arr, t_gc_object **gc)
{
	...
	new_obj = gc_alloc(sizeof(t_object), gc);
	if (!new_obj)
		return (print_error("Memory allocation failed for cylinder", *gc),
			false);
	...
	if (!assign_normal(arr[2], &new_obj->u_data.cylinder.direction, gc))
		return (print_error("Cylinder direction parse failed", *gc),
			false);
