...
static bool	set_amb_light(t_scene **scene, char **tokens, t_gc_object **gc_list)
{
	...
	if (!(*scene)->amb_light)
		return (print_error("Ambient light alloc failed", *gc_list), false);
...
/**
 * @brief Append a new light to the head of the linked list
 * 
 * @note Order doesn't matter
 * @note 'Elements which are defined by a capital letter can only be declared once in
the scene.' ---> Doesn't that mean that there would only be one source of light?? 
 */
