...
bool	assign_vector(char *coord, t_vec3 *vec, t_gc_object *gc_list)
{
	...
	tokens = ft_split(coord, ',');							// You dont have to gc_split() cause 'tokens' is a local variable, we free it before exiting the function.
	if (!tokens)
...
bool	assign_color(char *color, t_color *c, t_gc_object *gc_list)
{
	...
	tokens = ft_split(color, ',');							// You dont have to gc_split() cause 'tokens' is a local variable, we free it before exiting the function.
	if (!tokens)
