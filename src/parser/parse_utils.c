...
/*   Updated: 2025/05/07 11:58:40 by dchrysov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rgb_range_check(t_object **obj, char *rgb, t_gc_object **gc_list)
{
	...
	if (int_rgb[0] < 0 || int_rgb[0] > 255 || int_rgb[1] < 0 || int_rgb[1] > 255
		...
		print_error("RGB out of range [0, 255]", gc_list);
	(*obj)->rgb[0] = int_rgb[0];
	(*obj)->rgb[1] = int_rgb[1];
	(*obj)->rgb[2] = int_rgb[2];
	free_array(&tokens);
...
void	normal_vector_range_check(t_object **obj, char *vec, t_gc_object **gc_l)
{
	char		**tokens;
	int			d_normal[3];
	...
	if (d_normal[0] < -1.0 || d_normal[0] > 1.0 || d_normal[1] < -1.0
		...
		print_error("Normal vector out of range [-1.0, 1.0]", gc_l);
	if ((*obj)->type == PLANE)
	{
